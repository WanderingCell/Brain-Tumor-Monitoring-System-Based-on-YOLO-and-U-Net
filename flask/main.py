...
class VideoProcessingApp:
    def __init__(self, host='0.0.0.0', port=5000):
        ...
        self.port = port
        self.setup_routes()
        self.DeepSeek = 'sk-d90924728f6b47908f1ca173b17fc5fc'
        self.Qwen = 'sk-fedipknswnhpxslbykvgxyrmpvsxhezzbvrkvyeoqomytwkb'
        self.data = {}  # 存储接收参数
        self.paths = {
    ...
    def predictImg(self):
        ...
        if self.data["ai"] == 'DeepSeek' and self.data["status"] == 200:
            ...
            labels = self.process_list(results['labels'])
            confidences = self.process_list(results['confidences'])

            # 构造检测结果文本
            text = "我使用YOLO初步检测出以下病灶：\n"
            for label, conf in zip(labels, confidences):
                text += f"- {label}（置信度：{conf}）\n"

            # 添加请求建议的部分
            text += "\n请你基于上述检测结果，为我提供专业建议，包括：\n"
            text += "1. 建议检查的项目\n"
            text += "2. 推荐的手术或治疗方案\n"
            text += "3. 可选药物及其用途\n"
            text += "4. 康复注意事项\n"
            text += "请只给出你认为专业有用的建议即可。"
